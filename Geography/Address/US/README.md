...
Canonical code: `G:AD:US`. [Definition](index.json).

This record contains the former `G:AD` address fields and the US-style postal
...
display the corresponding names. Examples are `G:AD:RO:SH` for Shipping and
...
Type: `S:I:D:T:S` (string). Choices reference: `G:AD:RO`; only descendant leaves are acceptable. Example: `G:AD:RO:BI` (Billing). Role is not a free-text label.

[Field definition](Role.json).

### Address line 1 — `G:AD:US:L1`
...
Type: `S:I:D:T:S` (string). Choices reference: `G:CO`; only descendant leaves are acceptable. Example: `G:CO:US` (United States). The country list is sourced from a pinned ekkis/geo revision.

[Field definition](Country.json).

### Delivery instructions — `G:AD:US:DI`
